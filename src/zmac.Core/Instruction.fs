...
module Instruction =
    ...
    let instruction machine (InstructionAddress address) = 
      ...
      let decodeOperands address operandTypes =
        let rec loop operandTypes' address' acc =
          ...
          | operandType::tail -> 
              match operandType with
              | LargeOperand -> 
                  let operand = Constant (readWord machine (byteAddressToWordAddress address'))
                  loop tail (incrementByteAddressBy WordLength address') (acc @ [operand])
              | SmallOperand ->
                  let operand = Constant (readByte machine address')
      ...
      let operandsAddress = incrementByteAddressBy ((opcodeLength opcodeForm) + (operandTypesLength opcodeForm opcode)) instructionBaseAddress
      let operands = decodeOperands operandsAddress operandTypes
      let storeAddress = incrementByteAddressBy (operandsLength operandTypes) operandsAddress
